...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'checkbox':
					...
					if (array_key_exists("class",$value)) $fold.=$value['class']." ";
				...
				case 'social':
                    ...
                    if ($count < 2) {
                            $oldorder = 1;
                            $order = 1;
                            $output .= Options_Machine::optionsframework_social_function($value['id'],$value['std'],$oldorder,$order,$int);
                    } else {
                            $i = 0;
                            foreach ($slides as $slide) {
                                    $oldorder = $slide['order'];
                    ...
                    $output .= '</ul>';
                    $output .= '<a href="#" class="button sidebar_add_button">Add New Social</a></div>';
                    break;
	...
	/* ----------------------------------------------------------------------------------- */
	    ...
	    /*-----------------------------------------------------------------------------------*/
    /* Aquagraphite Sidebar - optionsframework_social_function */
    /*-----------------------------------------------------------------------------------*/

    public static function optionsframework_social_function($id,$std,$oldorder,$order,$int){
        ...
        $slider .= '<label>Title</label>';
        $slider .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][title]" id="'. $id .'_'.$order .'_social_title" value="'. stripslashes($val['title']) .'" />';

        $slider .= '<label>Link</label>';
        $slider .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_social_title" value="'. stripslashes($val['link']) .'" />';
